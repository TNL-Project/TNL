...
public:
   ...
   [[nodiscard]] constexpr static int getEntityDimension() {
      static_assert( TotalOrientation < getOrientationsCount() || EntityDimension < GridDimension, "Wrong number of total orientation." );
      if constexpr( TotalOrientation < getOrientationsCount() )
         return EntityDimension;
      else if constexpr ( EntityDimension < GridDimension )
         return NormalsGetter< int, EntityDimension + 1, GridDimension >::template getEntityDimension< TotalOrientation - getOrientationsCount() >();
      else {
         return -1;
      }
   }
   ...
   getNormalsByTotalOrientation()
   {
      ...
      else return NormalsGetter< Index, EntityDimension + 1, GridDimension >::
            template getNormalsByTotalOrientation< TotalOrientation - getOrientationsCount() >();
   }
...
private:
   ...
   struct IndexOfNormalsGetter {
      constexpr static int getIndex() {
         ...
         static_assert( OrientationIdx < getOrientationsCount(), "Index of entity orientation must be smaller than number of all orientations." );
