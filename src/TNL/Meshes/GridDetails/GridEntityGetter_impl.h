...
class GridEntityGetter<Meshes::Grid<1, Real, Device, Index>, GridEntity, EntityDimension> {
  public:
   ...
   __cuda_callable__ inline static GridEntity getEntity(const GridType& grid, const IndexType& index) {
      ...
      return GridEntity(grid, CoordinatesType(index), typename GridEntity::EntityOrientationType(0));
   }

   __cuda_callable__ inline static IndexType getEntityIndex(const GridType& grid, const GridEntity& entity) {
...
class GridEntityGetter<Meshes::Grid<2, Real, Device, Index>, GridEntity, 2> {
  public:
   ...
   __cuda_callable__ inline static GridEntity getEntity(const GridType& grid, const IndexType& index) {
      ...
      return GridEntity(grid, CoordinatesType(index % dimensions.x(), index / dimensions.x()), typename GridEntity::EntityOrientationType(0, 0));
   }
  ...
  public:
   ...
   __cuda_callable__ inline static GridEntity getEntity(const GridType& grid, const IndexType& index) {
      ...
      if (index < grid.numberOfNxFaces) {
         ...
         return GridEntity(grid, CoordinatesType(index % aux, index / aux), typename GridEntity::EntityOrientationType(1, 0));
      }
      const IndexType i = index - grid.numberOfNxFaces;
      const IndexType& aux = dimensions.x();
      return GridEntity(grid, CoordinatesType(i % aux, i / aux), typename GridEntity::EntityOrientationType(0, 1));
   }
...
class GridEntityGetter<Meshes::Grid<2, Real, Device, Index>, GridEntity, 0> {
  public:
   ...
   __cuda_callable__ inline static GridEntity getEntity(const GridType& grid, const IndexType& index) {
      ...
      const IndexType aux = dimensions.x() + 1;
      return GridEntity(grid, CoordinatesType(index % aux, index / aux), typename GridEntity::EntityOrientationType(0, 0));
   }
  ...
  public:
   ...
   __cuda_callable__ inline static GridEntity getEntity(const GridType& grid, const IndexType& index) {
      ...
      return GridEntity(grid,
                        CoordinatesType(index % dimensions.x(), (index / dimensions.x()) % dimensions.y(), index / (dimensions.x() * dimensions.y())),
...
class GridEntityGetter<Meshes::Grid<3, Real, Device, Index>, GridEntity, 2> {
  public:
   ...
   __cuda_callable__ inline static GridEntity getEntity(const GridType& grid, const IndexType& index) {
      ...
      if (index < grid.numberOfNxFaces) {
         const IndexType aux = dimensions.x() + 1;
         return GridEntity(grid, CoordinatesType(index % aux, (index / aux) % dimensions.y(), index / (aux * dimensions.y())),
                           typename GridEntity::EntityOrientationType(1, 0, 0));
      }
      if (index < grid.numberOfNxAndNyFaces) {
         const IndexType i = index - grid.numberOfNxFaces;
         const IndexType aux = dimensions.y() + 1;
         return GridEntity(grid, CoordinatesType(i % dimensions.x(), (i / dimensions.x()) % aux, i / (aux * dimensions.x())),
                           typename GridEntity::EntityOrientationType(0, 1, 0));
      }
      const IndexType i = index - grid.numberOfNxAndNyFaces;
      return GridEntity(grid, CoordinatesType(i % dimensions.x(), (i / dimensions.x()) % dimensions.y(), i / (dimensions.x() * dimensions.y())),
  ...
  public:
   ...
   __cuda_callable__ inline static GridEntity getEntity(const GridType& grid, const IndexType& index) {
      ...
      if (index < grid.numberOfDxEdges) {
         const IndexType aux = dimensions.y() + 1;
         return GridEntity(grid, CoordinatesType(index % dimensions.x(), (index / dimensions.x()) % aux, index / (dimensions.x() * aux)),
                           typename GridEntity::EntityOrientationType(0, 0, 0));
      }
      if (index < grid.numberOfDxAndDyEdges) {
         const IndexType i = index - grid.numberOfDxEdges;
         const IndexType aux = dimensions.x() + 1;
         return GridEntity(grid, CoordinatesType(i % aux, (i / aux) % dimensions.y(), i / (aux * dimensions.y())),
                           typename GridEntity::EntityOrientationType(0, 0, 0));
      }
      const IndexType i = index - grid.numberOfDxAndDyEdges;
      const IndexType aux1 = dimensions.x() + 1;
      const IndexType aux2 = dimensions.y() + 1;
      return GridEntity(grid, CoordinatesType(i % aux1, (i / aux1) % aux2, i / (aux1 * aux2)), typename GridEntity::EntityOrientationType(0, 0, 0));
   }

   __cuda_callable__ inline static IndexType getEntityIndex(const GridType& grid, const GridEntity& entity) {
      TNL_ASSERT_GE(entity.getCoordinates(), CoordinatesType(0, 0, 0), "wrong coordinates");
      TNL_ASSERT_LT(entity.getCoordinates(), grid.getDimensions() + CoordinatesType(1, 1, 1) - entity.getBasis(), "wrong coordinates");
...
class GridEntityGetter<Meshes::Grid<3, Real, Device, Index>, GridEntity, 0> {
  public:
   ...
   __cuda_callable__ inline static GridEntity getEntity(const GridType& grid, const IndexType& index) {
      ...
      return GridEntity(grid, CoordinatesType(index % auxX, (index / auxX) % auxY, index / (auxX * auxY)),
                        typename GridEntity::EntityOrientationType(0, 0, 0));
   }
