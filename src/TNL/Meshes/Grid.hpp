...
Grid< Dimension, Real, Device, Index >::getOrientedEntitiesCount() const noexcept
{
   ...
   const Index index = EntitiesOrientations::getTotalOrientationIndex( EntityDimension, EntityOrientationIdx ) + EntityDimension;
   return this->entitiesIndexesOffsets[ index+1 ] - this->entitiesIndexesOffsets[ index ];
...
Grid< Dimension, Real, Device, Index >::traverseAll( const CoordinatesType& from,
                                                     ...
                                                     FuncArgs... args ) const
{
   TNL_ASSERT_GE( from, CoordinatesType( 0 ), "Traverse rect must be in the grid sizes" );
   TNL_ASSERT_LE( to, this->getSizes(), "Traverse rect be in the grid sizes" );
...
auto
Grid< Dimension, Real, Device, Index >::
getEntityIndex( const Entity& entity, const CoordinatesType& offset ) const -> Index
{
...
{
   const IndexType totalOrientationIndex = EntitiesOrientations::getTotalOrientationIndex( OtherEntityDimension, otherEntityOrientationIdx );
...
getEntity( const Entity& entity, const CoordinatesType& offset,
           const NormalsType& otherEntityOrientation ) const -> EntityType< OtherEntityDimension >
{
   return EntityType< OtherEntityDimension >( *this, CoordinatesType( entity.getCoordinates() + offset ), otherEntityOrientation );
}
...
auto
Grid< Dimension, Real, Device, Index >::
getAdjacentEntityIndex( const Entity& entity ) const -> IndexType
{
...
getAdjacentFacesIndexes( const Entity& entity, CoordinatesType& closer, CoordinatesType& remoter ) const
{
   ...
   Algorithms::staticFor< int, begin, end >( [&] ( IndexType totalOrientationIndex ) mutable {
...
isBoundaryEntity( const Entity& entity ) const
{
   ...
   if constexpr( Entity::getEntityDimension() == Dimension ) {
      ...
      [&] ( Index i ) mutable {
         if( coordinates[ i ] == 0 ||
             coordinates[ i ] == this->getSizes()[ i ] - 1 )
...
setEntitiesIndexesOffsets()
{
   ...
   for( int entityDimension = 0; entityDimension <= Dimension; entityDimension++ )
   {
           ...
           entityOrientationIdx++, totalOrientationIdx++ )
      {
         ...
         Algorithms::staticFor< IndexType, 0, Dimension >(
            [&] ( Index i ) mutable { entitiesCount *= this->getSizes()[ i ] + normals[ i ]; } );
...
Grid< Dimension, Real, Device, Index >::fillProportions()
{
   ...
   while( i != Dimension ) {
      this->proportions[ i ] = this->spaceSteps[ i ] * this->sizes[ i ];
      i++;
   }
}
                                                     ...
                                                     FuncArgs... args ) const
{
   ...
   else
   {
      ...
      for( IndexType totalOrientationIdx = totalOrientationsBegin; totalOrientationIdx < totalOrientationsEnd; totalOrientationIdx++ ) {
         auto exec = [ = ] __cuda_callable__( GridEntityType& entity, const Grid& grid,
                                             FuncArgs... args ) mutable
         {
...
Grid< Dimension, Real, Device, Index >::forInteriorEntities( Func func, FuncArgs... args ) const
{
   ...
   if constexpr( EntityDimension == getMeshDimension() || EntityDimension == 0 )
   {
      ...
      Algorithms::ParallelForND< Device, false >::exec( GridEntityType( CoordinatesType( 1 ) ),
