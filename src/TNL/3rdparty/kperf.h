// The following is a file modified for TNL downloaded from:
//
// https://gist.github.com/ibireme/173517c208c7dc333ba962c1f0d67d12
//
// It serves for accessing various CPU counters in macOS.
// =============================================================================
...
// Counters
enum { TNL_KPERF_CYCLES, TNL_KPERF_INSTRUCTIONS, TNL_KPERF_BRANCHES, TNL_KPERF_BRANCH_MISSES };
static int counters_mapping[KPC_MAX_COUNTERS] = { 0 };
static u64 counters[KPC_MAX_COUNTERS] = { 0 };
static usize counter_map[KPC_MAX_COUNTERS] = { 0 };
static constexpr usize ev_count = sizeof(profile_events) / sizeof(profile_events[0]);
static u32 classes = 0;

// kperf initialization
static bool kperf_inited = false;

static int kperf_init(){
    if (kperf_inited) return 0;
    int ret = 0;
    // load dylib
    if (!lib_init()) {
        printf("Error: %s\n", lib_err_msg);
    ...
    // set counters mapping
    for (usize i = 0; i < ev_count; i++) {
       const event_alias *alias = profile_events + i;
       usize idx = counter_map[i];
       if( strcmp( alias->alias, "cycles" ) == 0 )
          counters_mapping[ TNL_KPERF_CYCLES ] = idx;
       if( strcmp( alias->alias, "instructions" ) == 0 )
          counters_mapping[ TNL_KPERF_INSTRUCTIONS ] = idx;
       if( strcmp( alias->alias, "branches" ) == 0 )
          counters_mapping[ TNL_KPERF_BRANCHES ] = idx;
       if( strcmp( alias->alias, "branch-misses" ) == 0 )
          counters_mapping[ TNL_KPERF_BRANCH_MISSES ] = idx;
    }
...
// This is for testing of counters_mapping
bool test_kperf_counters_mapping()
{
    for (usize i = 0; i < ev_count; i++) {
        const event_alias *alias = profile_events + i;
        usize idx = counter_map[i];
        if( strcmp( alias->alias, "cycles" ) == 0 )
            if( idx != counters_mapping[ TNL_KPERF_CYCLES ] )
            return false;
        if( strcmp( alias->alias, "instructions" ) == 0 )
            if( idx != counters_mapping[ TNL_KPERF_INSTRUCTIONS ] )
            return false;
        if( strcmp( alias->alias, "branches" ) == 0 )
            if( idx != counters_mapping[ TNL_KPERF_BRANCHES ] )
            return false;
        if( strcmp( alias->alias, "branch-misses" ) == 0 )
            if( idx != counters_mapping[ TNL_KPERF_BRANCH_MISSES ] )
            return false;
    }
    return true;
}

bool start_kperf_counters()
{
    // start counting
    int ret = 0;
    if ((ret = kpc_set_counting(classes))) {
...
bool get_kperf_counters( unsigned long long int& cpu_cycles,
                         ...
                         unsigned long long int& branch_misses )
{
    ...
    cpu_cycles = counters[ counters_mapping[ TNL_KPERF_CYCLES ] ];
    instructions = counters[ counters_mapping[ TNL_KPERF_INSTRUCTIONS ] ];
    branches = counters[ counters_mapping[ TNL_KPERF_BRANCHES ] ];
    branch_misses = counters[ counters_mapping[ TNL_KPERF_BRANCH_MISSES ] ];
    return true;
}
