...
bool
GEM< Real, Device, Index >::GEMdevice( Array& x, const TNL::String& pivoting, int verbose )
{
   ...
   *pom = -1;
   ...
   for( int colPointer = 0; colPointer < this->A.getNumColumns(); colPointer++ ) {
      ...
      if( pivoting == "yes" ) {
         // PIVOTING
         int reduceBlockSize = ( this->A.getNumColumns() - colPointer ) > 256
                                ? 256
      ...
      int blockSize = this->A.getNumRows() * ( this->A.getNumColumns() - colPointer ) > 256
                       ? 256
                       : this->A.getNumRows() * ( this->A.getNumColumns() - colPointer );
      ...
      GEMmainKernel< < < gridSize, blockSize > > >( devMat, device_vector.getView(), colPointer );
      cudaDeviceSynchronize();
      TNL_CHECK_CUDA_DEVICE;
   }
