...
__global__
void
showData( Real* data, int size, int processID )
{
   printf( "%d: [ ", processID );
         ...
         if( firstElementInRow != 0 ) {
            A->setElement( rowPointer,
                           ...
                           A->getElement( rowPointer, colPointer )
                              - firstElementInRow * mainRow[ colPointer - colPointerMain ] / pivot );
         }
      }
      else if( colPointer == colPointerMain && rowPointer == colPointerMain )
         printf( "Error, pivot is zero!\n" );
   }
   if( colPointer == colPointerMain && rowPointer + processID * A->getNumRows() != colPointerMain
       && rowPointer < A->getNumRows() && colPointer < A->getNumColumns() && mainRow[ 0 ] != 0
       && A->getElement( rowPointer, colPointerMain ) != 0 )
   {
      b[ rowPointer ] =
         b[ rowPointer ]
         - A->getElement( rowPointer, colPointerMain ) * mainRow[ A->getNumColumns() - colPointerMain ] / mainRow[ 0 ];
   }
}
...
__global__
void
GEMForwardPass( Matrix< Real, TNL::Devices::Cuda, int >* A,
                TNL::Containers::VectorView< Real, TNL::Devices::Cuda, int > b,
                int rowPointer )
{
      ...
      if( A->getElement( rowPointer, rowPointer ) != 0 ) {
         ...
         if( firstElementInRow != 0 ) {
            ...
            for( int i = rowPointer + 1; i < A->getNumColumns(); i++ ) {
               A->setElement( row, i, A->getElement( rowPointer, i ) - pivot * A->getElement( row, i ) / firstElementInRow );
            }
         }
      }
      else
...
__global__
void
GEMNormRows( Matrix< Real, TNL::Devices::Cuda, int >* A, TNL::Containers::VectorView< Real, TNL::Devices::Cuda, int > b )
{
   int row = threadIdx.x + blockIdx.x * blockDim.x;
   if( row < A->getNumRows() ) {
...
__global__
void
GEMBackwardPass( Matrix< Real, TNL::Devices::Cuda, int >* A,
                 TNL::Containers::VectorView< Real, TNL::Devices::Cuda, int > b,
                 int rowPointer )
{
   int row = threadIdx.x + blockIdx.x * blockDim.x;
...
__global__
void
showMatrix( Matrix< Real, Device, int > A )
{
   A.showMatrix();
}
...
__global__
void
GEMBlocks( Matrix< Real, TNL::Devices::Cuda, int >* A,
           TNL::Containers::VectorView< Real, TNL::Devices::Cuda, int > b,
           int mainBlockPointer )
{
         ...
         if( row > mainRowForBlock && mainRowForBlock < A->getNumRows() && rowPointer < A->getNumColumns() ) {
            if( A->getElement( mainRowForBlock, rowPointer ) != 0 ) {
               ...
               if( firstElementInRow != 0 ) {
                  ...
                  for( int i = rowPointer + 1; i < A->getNumColumns(); i++ ) {
                     A->setElement(
                        row, i, A->getElement( mainRowForBlock, i ) - pivot * A->getElement( row, i ) / firstElementInRow );
                  }
               }
            }
            else
         ...
         if( row < mainRowForBlock && mainRowForBlock < A->getNumRows() && rowPointer < A->getNumColumns() ) {
            if( A->getElement( mainRowForBlock, rowPointer ) != 0 ) {
               ...
               if( firstElementInRow != 0 ) {
                  ...
                  for( int i = rowPointer + 1; i < A->getNumColumns(); i++ ) {
                     A->setElement(
                        row, i, A->getElement( row, i ) - firstElementInRow * A->getElement( mainRowForBlock, i ) / pivot );
                  }
               }
            }
            else
...
__global__
void
GEMZeroing( Matrix< Real, TNL::Devices::Cuda, int >* A,
            TNL::Containers::VectorView< Real, TNL::Devices::Cuda, int > b,
            int mainBlockPointer )
{
       ...
       && mainBlockPointer < blockIdx.x )
   {
      ...
      for( int i = mainRowForThisRow + 1; i < A->getNumColumns(); i++ ) {
         A->setElement( row, i, pivot * A->getElement( row, i ) / firstElement - A->getElement( mainRowForThisRow, i ) );
      }
   }
}
