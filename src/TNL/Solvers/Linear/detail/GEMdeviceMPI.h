...
void
saveVec( Real* mainRow, Index size, int processID, Index colPointerMain )
{
...
template< typename Real, typename Index >
void
calculResultVector( Matrix< Real, TNL::Devices::Cuda, Index >& matrix,
...
bool
GEM< Real, Device, Index >::GEMdeviceMPI( Array& x, const TNL::String& pivoting, int verbose )
{
   ...
   // Main cycle for all rows across all MPI parts, vector x is the only one with full size on MPI, or use A.getNumColumns() for
   ...
   while( colPointerMain < x.getSize() ) {
      ...
      if( pivoting == "yes" ) {
         ...
         // Inicialising vectors for maximum and position (vectors important for multiple blocks, otherwise its vector with
         // length 1)
         ...
         if( fromRow != this->A.getNumRows() ) {
            ...
            int reduceBlockSize =
               ( this->A.getNumRows() - fromRow ) > 256 ? 256 : TNL::roundToMultiple( this->A.getNumRows() - fromRow, 32 );
            ...
            findPivot< < < reduceGridSize, reduceBlockSize > > >(
               devMat, fromRow, colPointerMain, outMax.getView(), outPos.getView() );
            cudaDeviceSynchronize();
            TNL_CHECK_CUDA_DEVICE;
         ...
         sendData[ 0 ] = outMax.getElement( 0 );
         sendData[ 1 ] = outPos.getElement( 0 );
         MPI_Barrier( MPI_COMM_WORLD );

         MPI_Allgather( sendData,
                        2,
                        TNL::Communicators::MPITypeResolver< Real >::getType(),
                        recvData,
                        2,
                        TNL::Communicators::MPITypeResolver< Real >::getType(),
                        MPI_COMM_WORLD );
         ...
         // else means that ProcessMax is in the main process that contains colPointerMain, in this case we need to do normal
         // pivoting so it swaps rows and fills mainRow normally.
         if( ProcessMax != colPointerMain / this->A.getNumRows() ) {
            ...
            if( colPointerMain / this->A.getNumRows() == processID ) {
               if( Possition != colPointer ) {
                  ...
                  int blockSize =
                     this->A.getNumColumns() - colPointerMain > 256 ? 256 : ( this->A.getNumColumns() - colPointerMain );
                  int gridSize = TNL::roundUpDivision( this->A.getNumColumns() - colPointerMain, blockSize );

                  swapRows< < < gridSize, blockSize > > >(
                     devMat, device_vector.getView(), colPointerMain, colPointer, Possition );
                  cudaDeviceSynchronize();
                  TNL_CHECK_CUDA_DEVICE;
               }
         ...
         // Onec more if the ProcessMax filled the mainRow, then the ProcessMax needs to switch this pivoting row with main
         // process. mainRowSwap is the colPointer of process colPointerMain/this->A.getNumRows()
         if( ProcessMax != colPointerMain / this->A.getNumRows() ) {
            Array mainRowSwapVec( this->A.getNumColumns() - colPointerMain + 1 );
            Real* mainRowSwap = mainRowSwapVec.getData();

            if( processID == ProcessMax ) {
               TNL::Communicators::MpiCommunicator::Recv(
                  mainRowSwap, this->A.getNumColumns() - colPointerMain + 1, colPointerMain / this->A.getNumRows(), 0 );
      ...
      else  // without pivoting
      {
         ...
         TNL::Communicators::MpiCommunicator::Bcast(
            data, this->A.getNumColumns() + 1, colPointerMain / this->A.getNumRows(), MPI_COMM_WORLD );
      ...
      //int numOfGrids = TNL::roundUpDivision( this->A.getNumRows() * (this->A.getNumColumns()-colPointerMain),
      //TNL::Cuda::getMaxGridSize() * 256 );

      int cudaGridSize( TNL::Cuda::getMaxGridSize() );
      int blockSize = this->A.getNumRows() * ( this->A.getNumColumns() - colPointerMain ) > 256
                       ? 256
                       : this->A.getNumRows() * ( this->A.getNumColumns() - colPointerMain );
   ...
   // (With MPI needs to send info into process 0 as main process with real result, rest processes has result as vector of
   // zeros)
