...
/**
 * \brief Third order [Heun's](https://en.wikipedia.org/wiki/List_of_Runge%E2%80%93Kutta_methods) method.
 *
...
struct Heun3
{
   ...
   getCoefficient( const std::size_t stage, const std::size_t i )
   {
      ...
      constexpr std::array< std::array< Value, Stages>, Stages > k_coefficients {
         ...
         std::array< Value, Stages >{     0.0, 2.0/3.0, 0.0 }
      };
      // clang-format on
   ...
   {
      constexpr std::array< Value, Stages > time_coefficients{ 0.0, 1.0 / 3.0, 2.0 / 3.0 };
