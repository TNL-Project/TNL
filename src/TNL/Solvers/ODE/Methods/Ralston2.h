...
/**
 * \brief Second order [Ralstons's](https://en.wikipedia.org/wiki/List_of_Runge%E2%80%93Kutta_methods) method.
 *
...
struct Ralston2
{
   ...
   getCoefficient( const std::size_t stage, const std::size_t i )
   {
      ...
      constexpr std::array< std::array< Value, Stages>, Stages > k_coefficients {
         ...
         std::array< Value, Stages >{ 2.0/3.0, 0.0 }
      };
      // clang-format on
   ...
   {
      constexpr std::array< Value, Stages > time_coefficients{ 0.0, 2.0 / 3.0 };
   ...
   {
      constexpr std::array< Value, Stages > update_coefficients{ 1.0 / 4.0, 3.0 / 4.0 };
