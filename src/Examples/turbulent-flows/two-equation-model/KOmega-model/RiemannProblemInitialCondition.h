...
//       pomocn8 t59da, kterou budu specialiyovat p5es r;zn0 dimenze gridu
      ...
      void setTurbulentEnergy(RealType NWUTurbulentEnergy,
                              RealType NEUTurbulentEnergy,
                              RealType SWUTurbulentEnergy,
                              RealType SEUTurbulentEnergy,
                              RealType NWDTurbulentEnergy,
                              RealType NEDTurbulentEnergy,
                              RealType SWDTurbulentEnergy,
                              RealType SEDTurbulentEnergy)
      {
         this->NWUTurbulentEnergy = NWUTurbulentEnergy;
         this->NEUTurbulentEnergy = NEUTurbulentEnergy;
         this->SWUTurbulentEnergy = SWUTurbulentEnergy;
         this->SEUTurbulentEnergy = SEUTurbulentEnergy;
         this->NWDTurbulentEnergy = NWDTurbulentEnergy;
         this->NEDTurbulentEnergy = NEDTurbulentEnergy;
         this->SWDTurbulentEnergy = SWDTurbulentEnergy;
         this->SEDTurbulentEnergy = SEDTurbulentEnergy;
      };

      void setDisipation(RealType NWUDisipation,
                         RealType NEUDisipation,
                         RealType SWUDisipation,
                         RealType SEUDisipation,
                         RealType NWDDisipation,
                         RealType NEDDisipation,
                         RealType SWDDisipation,
                         RealType SEDDisipation)
      {
         this->NWUDisipation = NWUDisipation;
         this->NEUDisipation = NEUDisipation;
         this->SWUDisipation = SWUDisipation;
         this->SEUDisipation = SEUDisipation;
         this->NWDDisipation = NWDDisipation;
         this->NEDDisipation = NEDDisipation;
         this->SWDDisipation = SWDDisipation;
         this->SEDDisipation = SEDDisipation;
      };
      ...
      void placeTurbulentEnergy(CompressibleConservativeVariables< MeshType >& conservativeVariables)
      {
      typedef typename MeshType::Cell CellType;
      typedef typename MeshType::CoordinatesType CoordinatesType;
      MeshType mesh = (* conservativeVariables.getDensity()).getMesh();
            ...
            if ( i < this->discontinuityPlacement[ 0 ] * mesh.getDimensions().x() )
               {
                  CellType cell(mesh, CoordinatesType(i));
                  cell.refresh();
                  (* conservativeVariables.getTurbulentEnergy()).setValue(cell, this->SWDTurbulentEnergy);
               }
            else
               {
                  CellType cell(mesh, CoordinatesType(i));
                  cell.refresh();
                  (* conservativeVariables.getTurbulentEnergy()).setValue(cell, this->SEDTurbulentEnergy);
               }
      };

      void placeDisipation(CompressibleConservativeVariables< MeshType >& conservativeVariables)
      {
      typedef typename MeshType::Cell CellType;
      typedef typename MeshType::CoordinatesType CoordinatesType;
      MeshType mesh = (* conservativeVariables.getDensity()).getMesh();
            ...
            if ( i < this->discontinuityPlacement[ 0 ] * mesh.getDimensions().x() )
               {
                  CellType cell(mesh, CoordinatesType(i));
                  cell.refresh();
                  (* conservativeVariables.getDisipation()).setValue(cell, this->SWDDisipation);
               }
            else
               {
                  CellType cell(mesh, CoordinatesType(i));
                  cell.refresh();
                  (* conservativeVariables.getDisipation()).setValue(cell, this->SEDDisipation);
               }
      };
      ...
      RealType  NWUTurbulentEnergy, NEUTurbulentEnergy, SWUTurbulentEnergy, SEUTurbulentEnergy;
      RealType  NWDTurbulentEnergy, NEDTurbulentEnergy, SWDTurbulentEnergy, SEDTurbulentEnergy;
      RealType  NWUDisipation, NEUDisipation, SWUDisipation, SEUDisipation, NWDDisipation, NEDDisipation, SWDDisipation, SEDDisipation;
                    ...
                    && ( k > this->discontinuityPlacement[ 2 ] * mesh.getDimensions().z() ) )
                     {
                        CellType cell(mesh, CoordinatesType(i,j,k));
                        cell.refresh();
                        (* conservativeVariables.getTurbulentEnergy()).setValue(cell, this->SWUTurbulentEnergy);
                     }
                  else
                  if ( ( i > this->discontinuityPlacement[ 0 ] * mesh.getDimensions().x() )
                    && ( j <= this->discontinuityPlacement[ 1 ] * mesh.getDimensions().y() )
                    && ( k > this->discontinuityPlacement[ 2 ] * mesh.getDimensions().z() ) )
                     {
                        CellType cell(mesh, CoordinatesType(i,j,k));
                        cell.refresh();
                        (* conservativeVariables.getTurbulentEnergy()).setValue(cell, this->SEUTurbulentEnergy);
                     }
                  else
                  if ( ( i <= this->discontinuityPlacement[ 0 ] * mesh.getDimensions().x() )
                    && ( j > this->discontinuityPlacement[ 1 ] * mesh.getDimensions().y() )
                    ...
                    && ( k > this->discontinuityPlacement[ 2 ] * mesh.getDimensions().z() ) )
                     {
                        CellType cell(mesh, CoordinatesType(i,j,k));
                        cell.refresh();
                        (* conservativeVariables.getDisipation()).setValue(cell, this->SWUDisipation);
                     }
                  else
                  if ( ( i > this->discontinuityPlacement[ 0 ] * mesh.getDimensions().x() )
                    && ( j <= this->discontinuityPlacement[ 1 ] * mesh.getDimensions().y() )
                    && ( k > this->discontinuityPlacement[ 2 ] * mesh.getDimensions().z() ) )
                     {
                        CellType cell(mesh, CoordinatesType(i,j,k));
                        cell.refresh();
                        (* conservativeVariables.getDisipation()).setValue(cell, this->SEUDisipation);
                     }
                  else
                  if ( ( i <= this->discontinuityPlacement[ 0 ] * mesh.getDimensions().x() )
                    && ( j > this->discontinuityPlacement[ 1 ] * mesh.getDimensions().y() )
...
*/
      ...
      RealType TurbulentEnergy( PointType Velocity, RealType intensity )
      {
         RealType turbulentEnergy;
         switch (Dimensions)
         {
            case 1: turbulentEnergy = 3.0 / 2.0 * intensity * intensity * ((std::pow(Velocity[ 0 ], 2 )));
                    break;
            case 2: turbulentEnergy = 3.0 / 2.0 * intensity * intensity * ((std::pow(Velocity[ 0 ], 2 ) + std::pow(Velocity[ 1 ], 2 )));
                    break;
            case 3: turbulentEnergy = 3.0 / 2.0 * intensity * intensity * ((std::pow(Velocity[ 0 ], 2 ) + std::pow(Velocity[ 1 ], 2 ) + std::pow(Velocity[ 2 ], 2 )));
                    break; // druhou mocninu ps8t jako sou4in
         }
         return turbulentEnergy;
      }
                                ...
                                const PointType& center = PointType( 0.0 ) )
      {
         ...
         variablesSetter->setEnergy(this->NWUEnergy,
                                    ...
                                    this->SWDEnergy,
                                    this->SEDEnergy);
         variablesSetter->setTurbulentEnergy(this->NWUTurbulentEnergy,
                                             this->NEUTurbulentEnergy,
                                             this->SWUTurbulentEnergy,
                                             this->SEUTurbulentEnergy,
                                             this->NWDTurbulentEnergy,
                                             this->NEDTurbulentEnergy,
                                             this->SWDTurbulentEnergy,
                                             this->SEDTurbulentEnergy);
         variablesSetter->setDisipation(this->NWUDisipation,
                                        this->NEUDisipation,
                                        this->SWUDisipation,
                                        this->SEUDisipation,
                                        this->NWDDisipation,
         ...
         variablesSetter->placeTurbulentEnergy(conservativeVariables);
