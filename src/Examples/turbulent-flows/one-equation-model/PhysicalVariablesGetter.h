...
class PhysicalVariablesGetter
{
   public:
      ...
      class TurbulentViscosityGetter : public Functions::Domain< Dimensions, Functions::MeshDomain >
      {
         public:
            ...
            TurbulentViscosityGetter( MeshFunctionPointer density,
                                      ...
                                      RealType specificLength )
            : density( density ), turbulentEnergy( turbulentEnergy ), viscosityConstant( viscosityConstant ), specificLength( specificLength ) {}
            ...
            RealType operator()( const EntityType& meshEntity,
                                 const RealType& time = 0.0 ) const
            {
               if( turbulentEnergy.template getData< DeviceType >()( meshEntity ) < 0.0 )
                  return 0;
               else
                  return density.template getData< DeviceType >()( meshEntity ) * viscosityConstant * specificLength 
                       * std::sqrt(turbulentEnergy.template getData< DeviceType >()( meshEntity ) );
            }
            
         protected:
            const MeshFunctionPointer density, turbulentEnergy;
            const RealType viscosityConstant, specificLength;
      };      
      ...
      void getTurbulentViscosity( const ConservativeVariablesPointer& conservativeVariables,
                                        MeshFunctionPointer& turbulentEnergy_no_rho,
                                  const RealType& viscosityConstant,
                                  const RealType& specificLength, 
