...
struct GridDistributor< TNL::Meshes::Grid< 2, Real, Device, Index > >
{
   ...
   void
   decompose( const GridType& grid, DistributedMesh< LocalMeshType >& mesh, int overlap )
   {
      ...
      verticesCount = 0;
      for( Index y = vert_begin.y() - ( rank_coordinates.y() > 0 ) * ( overlap + ( overlap > 0 ) );
           y < vert_end.y() + ( rank_coordinates.y() < rank_sizes.y() - 1 ) * overlap;
           y++ )
         for( Index x = vert_begin.x() - ( rank_coordinates.x() > 0 ) * ( overlap + ( overlap > 0 ) );
              x < vert_end.x() + ( rank_coordinates.x() < rank_sizes.x() - 1 ) * overlap;
              x++ )
            verticesCount++;
      cellsCount = 0;
      for( Index y = cell_begin.y() - ( rank_coordinates.y() > 0 ) * overlap;
           y < cell_end.y() + ( rank_coordinates.y() < rank_sizes.y() - 1 ) * overlap;
           y++ )
         for( Index x = cell_begin.x() - ( rank_coordinates.x() > 0 ) * overlap;
              x < cell_end.x() + ( rank_coordinates.x() < rank_sizes.x() - 1 ) * overlap;
              x++ )
            cellsCount++;
      ...
      //      for( Index y = vert_begin.y() - (rank_coordinates.y() > 0) * (overlap + (overlap>0)); y < vert_end.y() +
      //      (rank_coordinates.y() < rank_sizes.y() - 1) * overlap; y++ ) for( Index x = vert_begin.x() - (rank_coordinates.x()
      //      > 0) * (overlap + (overlap>0)); x < vert_end.x() + (rank_coordinates.x() < rank_sizes.x() - 1) * overlap; x++ )
      //         add_vertex( x, y );
      // - bottom left
      ...
      for( Index y = cell_begin.y(); y < cell_end.y(); y++ )
         for( Index x = cell_begin.x(); x < cell_end.x(); x++ )
            add_cell( x, y );
      // add ghost cells
      //      for( Index y = cell_begin.y() - (rank_coordinates.y() > 0) * overlap; y < cell_end.y() + (rank_coordinates.y() <
      //      rank_sizes.y() - 1) * overlap; y++ ) for( Index x = cell_begin.x() - (rank_coordinates.x() > 0) * overlap; x <
      //      cell_end.x() + (rank_coordinates.x() < rank_sizes.x() - 1) * overlap; x++ )
      //         add_cell( x, y );
      // - bottom left
      ...
      if( overlap > 0 ) {
         ...
         // compute mapping between old and new global indices (global indices on the distributed mesh != global indices on the
         // original grid)
   ...
   static std::map< Index, Index >
   renumberVertices( const GridType& grid, CoordinatesType rank_sizes )
   {
   ...
   static std::map< Index, Index >
   renumberCells( const GridType& grid, CoordinatesType rank_sizes )
   {
      Index idx = 0;
      std::map< Index, Index > result;
      const CoordinatesType local_size = grid.getSizes() / rank_sizes;
...
void
validateMesh( const Mesh& mesh, const Distributor& distributor, int ghostLevels )
{
   ...
   if( ghostLevels > 0 ) {
      ...
      Containers::Vector< Index, Device > cell_offsets( distributor.nproc + 1 );
      {
         ...
         TNL::MPI::Alltoall( cell_sendbuf.getData(), 1, cell_offsets.getData(), 1, distributor.communicator );
      }
...
struct TestEntitiesProcessor
{
   ...
   __cuda_callable__
   static void
   processEntity( const Mesh& mesh, UserData& userData, const Entity& entity )
   {
                       ...
                       const HostArray& expected_array_local )
{
   ...
   *meshPointer = mesh;
   ...
   auto f_boundary = [ view_boundary ] __cuda_callable__( typename MeshType::GlobalIndexType i ) mutable
   {
      view_boundary[ i ] += 1;
   };
   auto f_interior = [ view_interior ] __cuda_callable__( typename MeshType::GlobalIndexType i ) mutable
   {
      view_interior[ i ] += 1;
   };
   auto f_all = [ view_all ] __cuda_callable__( typename MeshType::GlobalIndexType i ) mutable
   {
      view_all[ i ] += 1;
   };
   auto f_ghost = [ view_ghost ] __cuda_callable__( typename MeshType::GlobalIndexType i ) mutable
   {
      view_ghost[ i ] += 1;
   };
   auto f_local = [ view_local ] __cuda_callable__( typename MeshType::GlobalIndexType i ) mutable
   {
      view_local[ i ] += 1;
   };
...
void
testSynchronizerOnDevice_global_indices( const MeshType& mesh )
{
...
template< typename Device, typename EntityType, typename MeshType >
void
testSynchronizerOnDevice_entity_centers( const MeshType& mesh )
{
...
#elif defined( __HIP__ )
   testSynchronizerOnDevice< Devices::Hip, typename Mesh::Cell >( mesh );
   testSynchronizerOnDevice< Devices::Hip, typename Mesh::Vertex >( mesh );
   if( mesh.template getGlobalIndices< 1 >().getSize() > 0 )
      testSynchronizerOnDevice< Devices::Hip, typename Mesh::Face >( mesh );
#endif
}
