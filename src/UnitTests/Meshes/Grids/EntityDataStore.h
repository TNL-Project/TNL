...
public:
   ...
   struct View
   {
      ...
      getEntity( const Index index )
      {
         ...
         return {
            coordinates, normals,         indices[ index ], calls[ index ], orientations[ index ], isBoundary[ index ] > 0,
            center,      measure[ index ]
         };
      }
   ...
   getView()
   {
      return { calls.getView(),        indices.getView(),    coordinates.getView(), normals.getView(),
               orientations.getView(), isBoundary.getView(), center.getView(),      measure.getView() };
   }
