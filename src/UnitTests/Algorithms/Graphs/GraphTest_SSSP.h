...
TYPED_TEST( GraphTest, test_BFS_larger )
{
   ...
   std::vector< VectorType > expectedDistances = {
      ...
      { -1.0, -1.0,  0.0, 0.8, -1.0,  2.1, 2.3, -1.0,  3.8, 5.6 },
      { -1.0, -1.0, -1.0, 0.0, -1.0, -1.0, 1.5, -1.0, -1.0, 4.8 },
      { -1.0,  3.7, -1.0, 1.5,  0.0, -1.0, 3.0,  0.9, -1.0, 3.1 },
      { -1.0, -1.0, -1.0, 4.3, -1.0,  0.0, 2.4, -1.0,  1.7, 3.6 },
      { -1.0, -1.0, -1.0, 4.0, -1.0, -1.0, 0.0, -1.0, -1.0, 3.3 },
      { -1.0,  4.1, -1.0, 1.9,  0.4, -1.0, 3.4,  0.0, -1.0, 2.2 },
      { -1.0, -1.0, -1.0, 2.6, -1.0, -1.0, 4.1, -1.0,  0.0, 1.9 },
   ...
   for( int start_node = 0; start_node < graph.getNodesCount(); start_node++ )
   {
      ...
      for( IndexType i = 0; i < graph.getNodesCount(); i++ )
         ASSERT_FLOAT_EQ(distances.getElement( i ), expectedDistances[ start_node ].getElement( i ) )
            ...
            << " distances: " << distances
            << " expectedDistances[ " << start_node << " ]: " << expectedDistances[ start_node ];
   }
}
