...
void
fetchAndSwapSorter( TNL::Containers::ArrayView< TYPE, TNL::Devices::Host > view )
{
   auto Cmp = [ = ]( const int i, const int j )
   {
      return view[ i ] < view[ j ];
   };
   auto Swap = [ = ]( int i, int j ) mutable
   {
...
void
fetchAndSwap_sortMiddle( TNL::Containers::ArrayView< int, TNL::Devices::Host > view, int from, int to )
{
   //auto Fetch = [=](int i){return view[i];};
   auto Cmp = [ = ]( const int i, const int j )
   {
      return view[ i ] < view[ j ];
   };
