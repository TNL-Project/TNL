...
template< typename GridType, typename DofType, int Dim = GridType::getMeshDimension() >
class Printer
{};
...
public:
   ...
   __cuda_callable__
   RealType
   operator()( const EntityType& meshEntity, const RealType& time = 0.0 ) const
   {
...
public:
   static void
   print_dof( int rank, GridType grid, DofType dof )
   {
      //print local dof
      int maxx = grid.getDimensions().x();
      int maxy = grid.getDimensions().y();
