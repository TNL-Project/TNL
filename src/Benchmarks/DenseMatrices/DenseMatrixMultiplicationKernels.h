...
template< int tileDim,
          ...
          Algorithms::Segments::ElementsOrganization organisation >
__global__
void
DenseMatrixProductKernelFinal( ResultMatrix resultMatrix,
                               const Matrix1 matrixA,
                               const Matrix2 matrixB,
                               const typename ResultMatrix::RealType matrixMultiplicator )
{
   ...
   if constexpr( organisation == Algorithms::Segments::ElementsOrganization::ColumnMajorOrder ) {
      row = by * tileDim + ty;  // Logic for column-major
      col = bx * tileDim + tx;
   }
   else {                       // For row-major order
      row = bx * tileDim + ty;  // Swap bx and by for row-major indexing
      col = by * tileDim + tx;
   }
   ...
   for( IndexType m = 0; m < ( tileDim + widthA - 1 ) / tileDim; ++m ) {
      ...
      // Load tileB from matrix B, correctly adjusting for transposition
   ...
   // Write the computed value back to the result matrix, considering column-major storage
   if( row < resultMatrix.getRows() && col < resultMatrix.getColumns() ) {
      resultMatrix( row, col ) = CValue * matrixMultiplicator;
   }
#endif  // __CUDACC__
