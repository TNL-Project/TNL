...
: public BenchmarkResult
{
   ...
   DenseMatricesResult( const HostMatrix& referenceResult,
                        const std::vector<BenchmarkMatrix>& benchmarkResults  )
   ...
   virtual std::vector< int > getColumnWidthHints() const override {
    std::vector<int> columnWidths = { 14, 14, 14, 6, 14, 14 };
    for (size_t i = 0; i < benchmarkResults.size(); i++) {
