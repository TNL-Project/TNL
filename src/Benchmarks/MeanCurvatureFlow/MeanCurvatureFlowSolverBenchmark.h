...
struct MeanCurvatureFlowSolverBenchmarkBase
{
   static void
   configSetup( TNL::Config::ConfigDescription& config )
   {
      ...
      config.addEntry< TNL::String >( "log-file", "Log file name.", "tnl-benchmark-mean-curvature-flow.log" );
      config.addEntry< TNL::String >( "output-mode", "Mode for opening the log file.", "overwrite" );
      config.addEntryEnum( "append" );
      config.addEntryEnum( "overwrite" );
      config.addEntry< int >( "loops", "Number of benchmarking loops to compute average time of benchmarked problem.", 1 );
      ...
      config.addEntry< int >( "z-size-step-factor",
                              "Factor determining the dimension grows over z axis. First size is min-z-dimension and each "
                              "following size is stepFactor*previousSize, up to max-z-dimension.",
                              2 );
      ...
      config.addEntry< double >( "alpha", "Alpha value in initial condition", 2 );
      config.addEntry< double >( "beta", "Beta value in initial condition", 10 );
      ...
      config.addEntry< double >( "final-time", "Final time of the simulation.", 0.1 );
      config.addEntry< int >( "max-iterations", "Maximum time iterations.", 0 );
   }

   bool
   setup( const TNL::Config::ParameterContainer& parameters, int dimension )
   {
...
template< int Dimension = 2, typename Real = double, typename Device = TNL::Devices::Host, typename Index = int >
struct MeanCurvatureFlowSolverBenchmark;

template< typename Real, typename Device, typename Index >
struct MeanCurvatureFlowSolverBenchmark< 1, Real, Device, Index > : public MeanCurvatureFlowSolverBenchmarkBase< Real, Index >
{
   ...
   virtual void
   init( const Index xSize ) = 0;

   virtual void
   exec( const Index xSize ) = 0;

   virtual bool
   writeGnuplot( const std::string& filename, const Index xSize ) const = 0;

   void
   init( const Index xSize, VectorType& ux, VectorType& aux )
   {
      ux.setSize( xSize );
      aux.setSize( xSize );
      ...
      auto init = [ = ] __cuda_callable__( Index i ) mutable
      {
         ...
         uxView[ i ] = exp( -alpha_ * x * x ) * sin( beta_ * x );
         //delta_ * ( 1.0 - TNL::sign( x*x / alpha_ - 1.0 ) );
      };
   ...
   writeGnuplot( const std::string& filename, const Vector& u, const Index xSize ) const
   {
      ...
      for( Index i = 0; i < xSize; i++ )
         out << i * hx - this->xDomainSize / 2. << " " << u.getElement( i ) << std::endl;
      return out.good();
   }
   ...
   {
      if( ! MeanCurvatureFlowSolverBenchmarkBase< Real, Index >::setup( parameters, Dimension ) )
         return false;
      ...
      for( Index xSize = this->minXDimension; xSize <= this->maxXDimension; xSize *= this->xSizeStepFactor ) {
         ...
         this->init( xSize );
         if( this->writeData ) {
            TNL::String fileName = TNL::String( "initial-" ) + this->scheme() + "-" + precision + "-" + this->implementation
...
template< typename Real, typename Device, typename Index >
struct MeanCurvatureFlowSolverBenchmark< 2, Real, Device, Index > : public MeanCurvatureFlowSolverBenchmarkBase< Real, Index >
{
   ...
   virtual void
   init( const Index xSize, const Index ySize ) = 0;

   virtual void
   exec( const Index xSize, const Index ySize ) = 0;

   virtual bool
   writeGnuplot( const std::string& filename, const Index xSize, const Index ySize ) const = 0;

   void
   init( const Index xSize, const Index ySize, VectorType& ux, VectorType& aux )
   {
      ...
      auto init = [ = ] __cuda_callable__( const CoordinatesType& idx ) mutable
      {
         ...
         uxView[ index ] = exp( -alpha_ * ( x * x + y * y ) ) * sin( beta_ * sqrt( x * x + y * y ) );
      };
...
template< typename Real, typename Device, typename Index >
struct MeanCurvatureFlowSolverBenchmark< 3, Real, Device, Index > : public MeanCurvatureFlowSolverBenchmarkBase< Real, Index >
{
   ...
   virtual void
   init( const Index xSize, const Index ySize, const Index zSize ) = 0;
