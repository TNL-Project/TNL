...
template< typename Real,
          typename Device,
          typename Index >
struct MeanCurvatureFlowSemiImplicitSolverBenchmarkFDMGrid< 1, Real, Device, Index >: public MeanCurvatureFlowSolverBenchmark< 1, Real, Device, Index >
{
...
template< typename Real,
          typename Device,
          typename Index >
struct MeanCurvatureFlowSemiImplicitSolverBenchmarkFDMGrid< 2, Real, Device, Index >: public MeanCurvatureFlowSolverBenchmark< 2, Real, Device, Index >
{
   ...
   void init( const Index xSize, const Index ySize )
   {
      ...
      TNL::Algorithms::parallelFor< Device >( 0, xSize * ySize,
   ...
   void exec( const Index xSize, const Index ySize )
   {
      ...
      while( start < this->finalTime && ( ! this->maxIterations || iterations < this->maxIterations ) )
     {
         ...
         this->grid.template forInteriorEntities<2>( [=] __cuda_callable__( const typename Grid::Cell& entity ) mutable {
            ...
            const Real a_c = ( 1.0 / q_view[ c ] * hx_sqr_inv +
                               1.0 / q_view[ w ] * hx_sqr_inv +
                               1.0 / q_view[ c ] * hy_sqr_inv +
                               1.0 / q_view[ s ] * hy_sqr_inv ) * q_bar_view[ c ] * timestep;
...
template< typename Real,
          typename Device,
          typename Index >
struct MeanCurvatureFlowSemiImplicitSolverBenchmarkFDMGrid< 3, Real, Device, Index >: public MeanCurvatureFlowSolverBenchmark< 3, Real, Device, Index >
{
