...
bool startBenchmark( TNL::Config::ParameterContainer& parameters )
{
   ...
   /*if( scheme == "fvm" )
   {
      ...
      {
         MeanCurvatureFlowSolverBenchmarkFVMParallelFor< Dimension, Real, Device > benchmark;
         return benchmark.runBenchmark( parameters );
      }
      if( implementation == "grid" )
      {
         MeanCurvatureFlowSolverBenchmarkFVMGrid< Dimension, Real, Device > benchmark;
         return benchmark.runBenchmark( parameters );
      }
...
int main(int argc, char* argv[])
{
   ...
   configSetup( config );
   MeanCurvatureFlowSolverBenchmark<>::configSetup( config );
