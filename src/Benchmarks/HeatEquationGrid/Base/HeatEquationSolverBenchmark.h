...
class HeatEquationSolverBenchmark {
   public:
      template<typename Real, typename Device>
      void exec(const typename HeatEquationSolver<Real>::Parameters& params) const;
...
TNL::Config::ConfigDescription HeatEquationSolverBenchmark::makeInputConfig() {
...
template<typename Real, typename Device>
void HeatEquationSolverBenchmark::exec(const typename HeatEquationSolver<Real>::Parameters& params) const {
...
template<typename Real, typename Device>
void HeatEquationSolverBenchmark::runBenchmark(TNL::Benchmarks::Benchmark<>& benchmark,
                                          const std::size_t minXDimension,
                                          const std::size_t maxXDimension,
                                          const std::size_t xStepSizeFactor,
                                          const std::size_t minYDimension,
   ...
   std::cout << "HeatEquation grid with (" + precision + ", host allocator " + device + ")" << std::endl;
   ...
   for(std::size_t xSize = minXDimension; xSize <= maxXDimension; xSize *= xStepSizeFactor) {
      for(std::size_t ySize = minXDimension; ySize <= maxXDimension; ySize *= yStepSizeFactor) {
         ...
         auto lambda = [=]() {
            typename HeatEquationSolver<Real>::Parameters params(xSize, ySize, xDomainSize, yDomainSize, alpha, beta, gamma, timeStep, finalTime, false, false);
...
int main(int argc, char* argv[]) {
   HeatEquationSolverBenchmark solver;

   auto config = HeatEquationSolverBenchmark::makeInputConfig();
