...
__global__
void
part2( const element* data,
       element* data2,
       Params< element >* params,
       Hist* hist,
       Length< element >* lengths )
{
   const int tx = threadIdx.x;
      ...
      while( bi >= 0 ) {
         ...
         if( ( to - from ) < ( sbsize - 16 ) ) {
            ...
            if( ( to - from >= 1 ) && ( lphase != 2 ) ) {
               ...
               TNL::Algorithms::Sorting::detail::bitonicSort_Block( view_data, view_adata, shared, []( element a, element b ) {
                  return a < b;
               } );
            }
            __syncthreads();
         ...
         if( tx == 0 ) {
            ...
            element mip = TNL::min( TNL::min( data[ from ], data[ to - 1 ] ), data[ ( from + to ) / 2 ] );
            element map = TNL::max( TNL::max( data[ from ], data[ to - 1 ] ), data[ ( from + to ) / 2 ] );
...
{
   if( ! init )
      throw std::runtime_error( "GPUQSort not initialized" );

   if( threads == 0 || blockscount == 0 || sbsize == 0 ) {
      threads = 1 << (int) std::round( std::log( size * TK + TM ) / std::log( 2.0 ) );
      blockscount = 1 << (int) std::round( std::log( size * MK + MM ) / std::log( 2.0 ) );
      sbsize = 1 << (int) std::round( std::log( size * SK + SM ) / std::log( 2.0 ) );
   }
   ...
   workset[ 0 ].pivot = ( TNL::min( TNL::min( pivot_vals[ 0 ], pivot_vals[ 1 ] ), pivot_vals[ 2 ] )
                          + TNL::max( TNL::max( pivot_vals[ 0 ], pivot_vals[ 1 ] ), pivot_vals[ 2 ] ) )
                      / 2;
   unsigned int worksize = 1;

   unsigned int blocks = blockscount / 2;
   if( blocks == 0 ) blocks = 1;  // Prevent division by zero
   unsigned totsize = size;
   unsigned int maxlength = ( size / blocks ) / 4;
   if( maxlength == 0 ) maxlength = 1;  // Prevent division by zero
   ...
   while( worksize < blocks ) {
      ...
      if( ws == 0 ) ws = 1;  // Prevent division by zero
   ...
   for( unsigned int i = 0; i < worksize; i++ )
      sortblocks[ i ] =
         ( ( workset[ i ].end - workset[ i ].beg ) << (int) std::round( std::log( MAXBLOCKS * 4 ) / std::log( 2.0 ) ) ) + i;
...
{
   static void
   sort( Containers::ArrayView< element, Devices::Cuda >& array )
   {
