      ...
      while( bi >= 0 ) {
         ...
         if( ( to - from ) < ( sbsize - 16 ) ) {
            ...
            if( ( to - from >= 1 ) && ( lphase != 2 ) ) {
               ...
               TNL::Algorithms::Sorting::detail::bitonicSort_Block( view_data, view_adata, sarray, []( unsigned int a, unsigned int b ) {
                  return a < b;
               } );
            }
            __syncthreads();
         ...
         if( tx == 0 ) {
            ...
            unsigned int mip = TNL::min( TNL::min( data[ from ], data[ to - 1 ] ), data[ ( from + to ) / 2 ] );
            unsigned int map = TNL::max( TNL::max( data[ from ], data[ to - 1 ] ), data[ ( from + to ) / 2 ] );
...
{
   static void
   sort( Containers::ArrayView< int, Devices::Cuda >& array )
   {
