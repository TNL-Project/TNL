   ...
   unsigned int phase )
{
   ...
   oh = 0;
   TNL::Backend::memcpy( doh, &oh, 4, TNL::Backend::MemcpyHostToDevice );
#endif
   ...
   if( size < 3 ) {
      element pivot;
      TNL::Backend::memcpy( &pivot, data.getData(), sizeof( element ), TNL::Backend::MemcpyDeviceToHost );
      workset[ 0 ].pivot = pivot;
   }
   else {
      element pivot_vals[ 3 ];
      TNL::Backend::memcpy( pivot_vals, data.getData(), 3 * sizeof( element ), TNL::Backend::MemcpyDeviceToHost );
