...
struct HeatEquationSolverBenchmark< 1, Real, Device, Index > : public HeatEquationSolverBenchmarkBase< Real, Index >
{
   ...
   runBenchmark( const TNL::Config::ParameterContainer& parameters )
   {
      ...
      for( Index xSize = this->minXDimension; xSize <= this->maxXDimension; xSize *= this->xSizeStepFactor ) {
         ...
         if( this->writeData ) {
            TNL::String fileName =
               TNL::String( "initial-" ) + this->implementation + "-" + TNL::convertToString( xSize ) + ".gplt";
         ...
         if( this->writeData ) {
            TNL::String fileName =
               TNL::String( "final-" ) + this->implementation + "-" + TNL::convertToString( xSize ) + ".gplt";
            writeGnuplot( fileName.data(), xSize );
         }
      }
      return true;
   ...
   runBenchmark( const TNL::Config::ParameterContainer& parameters )
   {
      ...
      for( Index xSize = this->minXDimension; xSize <= this->maxXDimension; xSize *= this->xSizeStepFactor ) {
         for( Index ySize = this->minYDimension; ySize <= this->maxYDimension; ySize *= this->ySizeStepFactor ) {
            ...
            if( this->writeData ) {
               TNL::String fileName = TNL::String( "initial-" ) + this->implementation + "-" + TNL::convertToString( xSize )
            ...
            benchmark.time< Device >( device, lambda );
            if( this->writeData ) {
               TNL::String fileName = TNL::String( "final-" ) + this->implementation + "-" + TNL::convertToString( xSize ) + "-"
                                    + TNL::convertToString( ySize ) + ".gplt";
               writeGnuplot( fileName.data(), xSize, ySize );
            }
         }
