...
struct HeatEquationSolverBenchmark
{
   ...
   writeGnuplot( const std::string& filename, const Index xSize ) const
   {
      ...
      for( Index i = 0; i < xSize; i++ )
         out << i * hx - this->xDomainSize / 2. << " " << this->ux.getElement( i ) << std::endl;
      return out.good();
   }
   ...
   runBenchmark( const TNL::Config::ParameterContainer& parameters )
   {
      ...
      if( dimension == 1 ) {
         for( Index xSize = minXDimension; xSize <= maxXDimension; xSize *= xSizeStepFactor ) {
            ...
            if( writeData ) {
               TNL::String fileName =
                  TNL::String( "initial-" ) + implementation + "-" + TNL::convertToString( xSize ) + ".gplt";
            ...
            if( writeData ) {
               TNL::String fileName = TNL::String( "final-" ) + implementation + "-" + TNL::convertToString( xSize ) + ".gplt";
               writeGnuplot( fileName.data(), xSize );
            }
         }
      }
