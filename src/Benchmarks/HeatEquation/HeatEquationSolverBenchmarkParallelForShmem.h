...
template< int Size = 1,
          ...
          typename Index = int >
struct HeatEquationSolverBenchmarkParallelForShmem : public HeatEquationSolverBenchmark< Real, Device, Index >
{
   void exec( const Index xSize, const Index ySize )
   {
      ...
      while( start < this->finalTime && ( ! this->maxIterations || iterations < this->maxIterations ) )
      {
         ...
         auto next = [=] __cuda_callable__( Index i, Index j ) mutable
         {
            ...
            UnrolledStaticVector< Size * sizeof( Real ) / sizeof( float ), float > v( 1 );
            element += v.getL1Norm();
