...
template< int Dimension, typename Index, typename Device = TNL::Devices::Host, typename = std::enable_if_t< ( Dimension > 0 ) > >
class NdGrid
{
public:
   NdGrid() = default;
   ...
   void
   traverseAll( Function function, FunctionArgs... args ) const noexcept
   {
...
struct HeatEquationSolverBenchmarkNdGrid : public HeatEquationSolverBenchmark< Real, Device, Index >
{
   ...
   exec( const Index xSize, const Index ySize ) override
   {
      ...
      while( start < this->finalTime && ( ! this->maxIterations || iterations < this->maxIterations ) ) {
         ...
         grid.traverse( { 1, 1 }, { grid.getEndIndex( 0 ) - 1, grid.getEndIndex( 1 ) - 1 }, { direction }, next );
