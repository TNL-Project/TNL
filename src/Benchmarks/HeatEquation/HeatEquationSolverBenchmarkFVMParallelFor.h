...
struct HeatEquationSolverBenchmarkFVMParallelFor< 1, Real, Device, Index >
...
template< typename Real, typename Device, typename Index >
struct HeatEquationSolverBenchmarkFVMParallelFor< 2, Real, Device, Index >
: public HeatEquationSolverBenchmark< 2, Real, Device, Index >
{
   ...
   bool
   writeGnuplot( const std::string& filename, const Index xSize, const Index ySize, const VectorType& u ) const
   {
...
template< typename Real, typename Device, typename Index >
struct HeatEquationSolverBenchmarkFVMParallelFor< 3, Real, Device, Index >
   ...
   exec( const Index xSize, const Index ySize, const Index zSize )
   {
      ...
      while( start < this->finalTime && ( ! this->maxIterations || iterations < this->maxIterations ) ) {
         ...
         TNL::Algorithms::parallelFor< Device >(
            CoordinatesType{ 1, 1, 1 }, CoordinatesType{ xSize, ySize - 1, zSize - 1 }, x_gradients );
         ...
         TNL::Algorithms::parallelFor< Device >(
            CoordinatesType{ 1, 1, 1 }, CoordinatesType{ xSize - 1, ySize, zSize - 1 }, y_gradients );
         ...
         TNL::Algorithms::parallelFor< Device >(
            CoordinatesType{ 1, 1, 1 }, CoordinatesType{ xSize - 1, ySize - 1, zSize }, z_gradients );
         ...
         TNL::Algorithms::parallelFor< Device >(
            CoordinatesType{ 1, 1, 1 }, CoordinatesType{ xSize - 1, ySize - 1, zSize - 1 }, update );
         this->ux.swap( this->aux );
         start += timestep;
         iterations++;
      }
