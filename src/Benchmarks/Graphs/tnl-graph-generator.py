...
def generate_random_graph(nodes, edges, edge_weights_distribution):
    ...
    for u, v in selected_edges:
        if edge_weights_distribution == "uniform":
            ...
            raise ValueError(
                "Invalid edge_weights_distribution. Must be 'uniform' or 'normal'."
            )
...
def main():
    ...
    parser.add_argument(
        "--nodes", type=int, required=True, help="Number of nodes in the graph"
    )
    parser.add_argument(
        "--edges", type=int, required=True, help="Number of edges in the graph"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Name of the file to write the edge list",
    )

    args = parser.parse_args()

    graph = generate_random_graph(
        args.nodes, args.edges, args.edge_weights_distribution
    )
