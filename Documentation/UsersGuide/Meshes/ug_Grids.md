...
The grid entity stores the vector with packed normals, the basis vector is always computed on the fly. So whenever it possible, using the the normals vector is preferred for better performance.
...
The whoel example consists of the following steps:
...
Writers help to export data linked with a grid to some of standard formats like [VTK, VTI](https://kitware.github.io/vtk-examples/site/VTKFileFormats/) or [Gnuplot](http://www.gnuplot.info/). The following example shows the use of the grid writers:
